...
void main()
{
    textCoord = position_in;
	position = vec4( position_in, 0.0, 1.0 );
...
//--------------------------------------------------------------------------------------------------------
// En m'appuyant sur : 
// https://www.cs.cornell.edu/courses/cs4620/2017sp/cs4621/lecture08/exhibit00.html
//--------------------------------------------------------------------------------------------------------
var fullscreen_fS =
`#version 300 es
precision highp float;
...
uniform int orientation; // 0:x; 1:y


void main()
{
    float convolution[5] = float[] (0.090404,0.241812,0.335568,0.241812,0.090404); 
    vec4 color = vec4(0.0,0.0,0.0,0.0); 
    vec2 flouCoord[5];
    float texSizeX = 1.0 / textWidth;
    float texSizeY = 1.0 / textHeight;
    if (orientation==0) 
    {
        for (int i=-2;i<=2;i++) 
        {
            flouCoord[i+2] = textCoord + vec2(texSizeX*float(i),0.0);
        }
    }
    else
    {
        for (int i=-2;i<=2;i++) 
        {
            flouCoord[i+2] = textCoord + vec2(0.0,texSizeY*float(i));
        }       
    }

    for (int i=0;i<5;i++)
    {
        color += texture(uSampler,flouCoord[i])*convolution[i];
    }
...
var textFBO2 = null; 
var fboTexWidth = 512; 
var fboTexHeight = 512;
var fullscreen_shaderProgram = null; // Shader qui dessine le Quad à l'écran
var prg_flat=null;
...
function init_wgl()
{
    ...
    let bleuCiel = "images/bleu-ciel.jpg";
    ...
    fullscreen_shaderProgram = ShaderProgram(fullscreen_vertexShader, fullscreen_fS, 'fullscreen shader');
    ...
    textFBO1 = gl.createTexture();
	gl.bindTexture(gl.TEXTURE_2D, textFBO1);
	...
	gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textFBO1, 0);
	gl.drawBuffers([gl.COLOR_ATTACHMENT0]);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null)
    

    // fbo2 + textFBO2
    textFBO2 = gl.createTexture();
	gl.bindTexture(gl.TEXTURE_2D, textFBO2);
	gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, fboTexWidth, fboTexHeight, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
	gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
	gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
	gl.bindTexture(gl.TEXTURE_2D, null);
	...
	gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textFBO2, 0);
	gl.drawBuffers([gl.COLOR_ATTACHMENT0]);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null)
    ...
    io_tex = Texture2d();
    io_tex.load("/images/LuneJupiter/io.jpg",gl.RGB8);
    io_rend = io.renderer(1,2,3);
    
    let europe = Mesh.Sphere(20); 
    europe_tex = Texture2d();
    europe_tex.load("/images/LuneJupiter/europe.jpg",gl.RGB8);
    europe_rend = europe.renderer(1,2,3);

    let ganymede = Mesh.Sphere(20) ;
    ganymede_tex = Texture2d();
    ganymede_tex.load("/images/LuneJupiter/ganymede.png",gl.RGB8);
...
{	
    ...
    gl.clear(gl.COLOR_BUFFER_BIT);
        ...
        Uniforms.uViewMatrix = ewgl.scene_camera.get_view_matrix();
        Uniforms.uSensMatrix=cam;
        Uniforms.color = Vec4(1.0, 0.75, 0.1,1.0);
    ...
    for(let pass=0; pass<15; pass++)
    {
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D,textFBO1);
        // Rendu plat, pour glow
        prg_flat.bind();
        gl.bindFramebuffer(gl.FRAMEBUFFER, fbo2); 
        gl.viewport(0, 0, fboTexWidth, fboTexHeight);
        gl.clearColor(0.0,0.0,0.0,1.0);
        gl.clear(gl.COLOR_BUFFER_BIT);              
            // rendu du fbo1 dans fbo2, orientation 1 => blur y
            fullscreen_shaderProgram.bind();    
            Uniforms.uSampler = textFBO2;
            Uniforms.textWidth = fboTexWidth;
            Uniforms.textHeight = fboTexWidth;
            Uniforms.orientation = 1;
            gl.bindVertexArray( vao );
        ...
        gl.clearColor(0.0,0.0,0.0,1.0);
        gl.clear(gl.COLOR_BUFFER_BIT);              
            // rendu du fbo2 dans fbo1, orientation 0 => blur x
            fullscreen_shaderProgram.bind();    
            Uniforms.uSampler = textFBO1;
            Uniforms.textWidth = fboTexWidth;
            Uniforms.textHeight = fboTexWidth;
            Uniforms.orientation = 0;
            gl.bindVertexArray( vao );
            gl.drawArrays( gl.TRIANGLES, 0, 6 );
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
        gl.viewport(0, 0, gl.canvas.width, gl.canvas.height);    
